...
async def _next_alias(db: AsyncSession, user_id: int) -> str:
    # Aliases are globally unique (the proxy route has no user context to scope by),
    # so the suggestion has to avoid every alias in the system, not just this user's.
    rows = (await db.execute(select(ApiKey.alias))).scalars().all()
...
async def create_key(
    ...
    if not ALIAS_RE.match(alias):
        ...
        await db.execute(select(ApiKey).where(ApiKey.alias == alias))
    ).scalar_one_or_none():
        error = f"The alias '{alias}' is already taken. Aliases are global, try another."
...
async def rename_alias(
    ...
    if ALIAS_RE.match(new_alias):
        clash = (
            await db.execute(
                select(ApiKey).where(ApiKey.alias == new_alias, ApiKey.id != key.id)
            )
        ).scalar_one_or_none()
        if clash is None:
            key.alias = new_alias
